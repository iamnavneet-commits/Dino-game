 ...
 *
 * Architecture:
 * 1) Input handling (keyboard events)
 * 2) Update step (physics, spawning, scoring, collision)
 * 3) Render step (draw scene and overlays)
 *
 * The game uses requestAnimationFrame and a capped delta time for smooth, stable gameplay.
 */
...
if (!canvas || !scoreLabel || !highScoreLabel) {
  throw new Error('Missing required game DOM elements.');
}
...
function update(deltaTime) {
  ...
  gameState.speed = BASE_SPEED + gameState.elapsed * 12;
...
function render() {
  ...
  ctx.fillRect(player.x + 30, player.y + 6, 9, 9);
  ctx.fillStyle = '#fff';
  ctx.fillRect(player.x + 30, player.y + 14, 4, 4);
